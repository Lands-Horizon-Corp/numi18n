...
var EN001Locale = NumI18NLocale{
	LocaleFormatter: &EnglishWorldFormatter{},
	Currency: Currency{
		Name:     "Dollar",
		Plural:   "Dollars",
		Singular: "Dollar",
...
// EnglishWorldFormatter handles English (World) formatting
type EnglishWorldFormatter struct{}

func (f *EnglishWorldFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *EnglishWorldFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *EnglishWorldFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *EnglishWorldFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *EnglishWorldFormatter) FormatNegative(result, negativeWord string) string {
...
func (f *EnglishWorldFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	...
	// Format with thousand separators, currency symbol, and symbol prefix (true for World English)
