...
var GBLocale = NumI18NLocale{
	LocaleFormatter: &EnglishUKFormatter{},
	Currency: Currency{
		Name:     "Pound Sterling",
...
// EnglishUKFormatter handles English (United Kingdom) formatting
type EnglishUKFormatter struct{}

func (f *EnglishUKFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *EnglishUKFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *EnglishUKFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *EnglishUKFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *EnglishUKFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *EnglishUKFormatter) ChopDecimal(d decimal.Decimal, precision int) decimal.Decimal {
	return d.Truncate(int32(precision))
}

func (f *EnglishUKFormatter) FormatDecimalNumber(amount float64) string {
	return FormatDecimalWithSeparators(amount, ",")
}

func (f *EnglishUKFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
	...
	// Format with thousand separators, currency symbol, and symbol prefix (true for UK) - always shows 2 decimal places
