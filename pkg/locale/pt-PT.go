...
// PTPTLocale represents the Portuguese (Portugal) locale
var PTPTLocale = NumI18NLocale{
	Currency: Currency{
		Name:     "Euro",
		Plural:   "Euros",
		Singular: "Euro",
	...
	LocaleFormatter: &PortuguesePortugalFormatter{},
}

// PortuguesePortugalFormatter handles Portuguese (Portugal) formatting
type PortuguesePortugalFormatter struct{}

func (f *PortuguesePortugalFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *PortuguesePortugalFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *PortuguesePortugalFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *PortuguesePortugalFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *PortuguesePortugalFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *PortuguesePortugalFormatter) ChopDecimal(value decimal.Decimal, precision int) decimal.Decimal {
	return value.Truncate(int32(precision))
}


func (f *PortuguesePortugalFormatter) FormatDecimalNumber(amount float64) string {
	return DefaultFormatDecimalNumber(amount, ",", ".")
}
func (f *PortuguesePortugalFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
