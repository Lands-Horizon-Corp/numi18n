...
func (f *EnglishFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
...
func (f *EnglishFormatter) FormatDecimalNumber(amount float64) string {
...
func (f *EnglishFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	...
	// For US English: currency symbol comes before the number
...
func (f *EnglishFormatter) formatWithThousandSeparators(numStr string) string {
	if len(numStr) <= 3 {
		return numStr
	}
