...
var CAFRLocale = NumI18NLocale{
	...
	LocaleFormatter: &FrenchCanadaFormatter{},
}

// FrenchCanadaFormatter handles French Canada formatting
type FrenchCanadaFormatter struct{}

func (f *FrenchCanadaFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *FrenchCanadaFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *FrenchCanadaFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *FrenchCanadaFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *FrenchCanadaFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *FrenchCanadaFormatter) ChopDecimal(value decimal.Decimal, precision int) decimal.Decimal {
	return value.Round(int32(precision))
}

func (f *FrenchCanadaFormatter) FormatDecimalNumber(amount float64) string {
	return FormatFrenchDecimal(amount)
}

func (f *FrenchCanadaFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
