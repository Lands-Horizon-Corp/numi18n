...
var PTCVLocale = NumI18NLocale{
	...
	LocaleFormatter: &PortugueseCapeVerdeFormatter{},
}

// PortugueseCapeVerdeFormatter handles Portuguese (Cape Verde) formatting
type PortugueseCapeVerdeFormatter struct{}

func (f *PortugueseCapeVerdeFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *PortugueseCapeVerdeFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *PortugueseCapeVerdeFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *PortugueseCapeVerdeFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *PortugueseCapeVerdeFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *PortugueseCapeVerdeFormatter) ChopDecimal(value decimal.Decimal, precision int) decimal.Decimal {
	return value.Truncate(int32(precision))
}

func (f *PortugueseCapeVerdeFormatter) FormatDecimalNumber(amount float64) string {
	return FormatPolishDecimal(amount)
}
func (f *PortugueseCapeVerdeFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
