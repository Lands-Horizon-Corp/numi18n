...
var ZHTWLocale = NumI18NLocale{
	...
	NumberWordsMapping: []NumberWordMapping{
		...
		{Number: 10000000, Value: "一千萬"},
		{Number: 1000000, Value: "一百萬"},
		{Number: 100000, Value: "十萬"},
		{Number: 10000, Value: "一萬"},
	...
	LocaleFormatter: &ChineseTaiwanFormatter{},
}

// ChineseTaiwanFormatter handles Chinese (Taiwan) formatting
type ChineseTaiwanFormatter struct{}

func (f *ChineseTaiwanFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *ChineseTaiwanFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	// Chinese doesn't have plural forms for currency, always use the same form
	return result + currencyPlural
}

func (f *ChineseTaiwanFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + andText + fractionalWords
}

func (f *ChineseTaiwanFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
	// Chinese doesn't have plural forms for fractions, always use the same form
	return result + fractionPlural
}

func (f *ChineseTaiwanFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + result
}

func (f *ChineseTaiwanFormatter) ChopDecimal(value decimal.Decimal, places int) decimal.Decimal {
	return value.Truncate(int32(places))
}

func (f *ChineseTaiwanFormatter) FormatDecimalNumber(amount float64) string {
	return FormatAsianDecimal(amount)
}
func (f *ChineseTaiwanFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
