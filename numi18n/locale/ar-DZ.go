...
var ARDZLocale = NumI18NLocale{
	...
	LocaleFormatter: &ArabicAlgeriaFormatter{},
}

// ArabicAlgeriaFormatter handles Arabic Algeria (ar-DZ) formatting
type ArabicAlgeriaFormatter struct{}

func (f *ArabicAlgeriaFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *ArabicAlgeriaFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *ArabicAlgeriaFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *ArabicAlgeriaFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *ArabicAlgeriaFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *ArabicAlgeriaFormatter) ChopDecimal(amount decimal.Decimal, precision int) decimal.Decimal {
	if precision < 0 {
		precision = 2
	}
...
func (f *ArabicAlgeriaFormatter) FormatDecimalNumber(amount float64) string {
	return FormatAngloDecimal(amount)
}
func (f *ArabicAlgeriaFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
