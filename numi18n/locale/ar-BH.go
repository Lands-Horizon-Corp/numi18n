...
var ARBHLocale = NumI18NLocale{
	...
	LocaleFormatter: &ArabicBahrainFormatter{},
}

// ArabicBahrainFormatter handles Arabic Bahrain (ar-BH) formatting
type ArabicBahrainFormatter struct{}

func (f *ArabicBahrainFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *ArabicBahrainFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *ArabicBahrainFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *ArabicBahrainFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *ArabicBahrainFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *ArabicBahrainFormatter) ChopDecimal(amount decimal.Decimal, precision int) decimal.Decimal {
	if precision < 0 {
		precision = 2
	}
...
func (f *ArabicBahrainFormatter) FormatDecimalNumber(amount float64) string {
	return FormatAngloDecimal(amount)
}
func (f *ArabicBahrainFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
