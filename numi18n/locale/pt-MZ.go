...
var PTMZLocale = NumI18NLocale{
	...
	LocaleFormatter: &PortugueseMozambiqueFormatter{},
}

// PortugueseMozambiqueFormatter handles Portuguese (Mozambique) formatting
type PortugueseMozambiqueFormatter struct{}

func (f *PortugueseMozambiqueFormatter) FormatNumber(number int64, targetLocale NumI18NLocale) string {
	return ConvertToWordsWithExactMappingInt64(number, targetLocale)
}

func (f *PortugueseMozambiqueFormatter) FormatCurrency(result string, wholePart int64, currencyName, currencyPlural string) string {
	if wholePart == 1 {
...
func (f *PortugueseMozambiqueFormatter) FormatFractional(result, fractionalWords string, andText string) string {
	return result + " " + andText + " " + fractionalWords
}

func (f *PortugueseMozambiqueFormatter) FormatFractionalCurrency(result string, fractionalValue int64, fractionName, fractionPlural string) string {
...
func (f *PortugueseMozambiqueFormatter) FormatNegative(result, negativeWord string) string {
	return negativeWord + " " + result
}

func (f *PortugueseMozambiqueFormatter) ChopDecimal(value decimal.Decimal, precision int) decimal.Decimal {
	return value.Truncate(int32(precision))
}

func (f *PortugueseMozambiqueFormatter) FormatDecimalNumber(amount float64) string {
	return FormatPolishDecimal(amount)
}
func (f *PortugueseMozambiqueFormatter) FormatDecimalNumberWithCurrency(amount float64, targetLocale NumI18NLocale, overrideOptions *OverrideOptions) string {
	// Get currency symbol
