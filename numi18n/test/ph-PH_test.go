package test

import (
	"testing"

	"github.com/Lands-Horizon-Corp/numi18n/numi18n"
)

func TestToWords_Filipino_Numbers(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Zero",
			amount: 0,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Zero",
		},
		{
			name:   "Single digit",
			amount: 5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Lima",
		},
		{
			name:   "Ten",
			amount: 10,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Sampu",
		},
		{
			name:   "Eleven",
			amount: 11,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Labing-isa",
		},
		{
			name:   "Twenty",
			amount: 20,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Dalawampu",
		},
		{
			name:   "Twenty one",
			amount: 21,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Dalawampu Isa",
		},
		{
			name:   "One hundred (special case)",
			amount: 100,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang daan",
		},
		{
			name:   "One hundred twenty three",
			amount: 123,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang daan Dalawampu Tatlo",
		},
		{
			name:   "One thousand",
			amount: 1000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Libo",
		},
		{
			name:   "Ten thousand",
			amount: 10000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Sampu Libo",
		},
		{
			name:   "One hundred thousand",
			amount: 100000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang daan Libo",
		},
		{
			name:   "One million",
			amount: 1000000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Milyun",
		},
		{
			name:   "Complex number",
			amount: 1234567,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Milyun Dalawa Daan Tatlong pu Apat Libo Lima Daan Animnapu Pito",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToWords(tt.amount)
			if result != tt.expected {
				t.Errorf("ToWords() = %v, want %v", result, tt.expected)
			}
		})
	}
}

func TestToWords_Filipino_Currency(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "One peso",
			amount: 1,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "Isa Peso",
		},
		{
			name:   "Five pesos",
			amount: 5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "Lima Piso",
		},
		{
			name:   "One hundred pesos",
			amount: 100,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "Isang daan Piso",
		},
		{
			name:   "One thousand pesos",
			amount: 1000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "Isang Libo Piso",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToWords(tt.amount)
			if result != tt.expected {
				t.Errorf("ToWords() = %v, want %v", result, tt.expected)
			}
		})
	}
}

func TestToWords_Filipino_Decimals(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Zero point five (fraction as percentage)",
			amount: 0.5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Zero at Limampu", // 0.5 = 0 and 50/100
		},
		{
			name:   "One point two five (fraction as percentage)",
			amount: 1.25,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Isa at Dalawampu Lima", // 1.25 = 1 and 25/100
		},
		{
			name:   "Three point one four (fraction as percentage)",
			amount: 3.14,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Tatlo at Labing-apat", // 3.14 = 3 and 14/100
		},
		{
			name:   "Ten point zero one (fraction as percentage)",
			amount: 10.01,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Sampu at Isa", // 10.01 = 10 and 1/100
		},
		{
			name:   "Hundred point five (fraction as percentage)",
			amount: 100.5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Isang daan at Limampu", // 100.5 = 100 and 50/100
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToWords(tt.amount)
			if result != tt.expected {
				t.Errorf("ToWords() = %v, want %v", result, tt.expected)
			}
		})
	}
}

func TestToWords_Filipino_Negative(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Negative one",
			amount: -1,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					NegativeWord: true,
				},
			},
			expected: "minus Isa",
		},
		{
			name:   "Negative five",
			amount: -5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					NegativeWord: true,
				},
			},
			expected: "minus Lima",
		},
		{
			name:   "Negative hundred",
			amount: -100,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					NegativeWord: true,
				},
			},
			expected: "minus Isang daan",
		},
		{
			name:   "Negative thousand",
			amount: -1000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					NegativeWord: true,
				},
			},
			expected: "minus Isang Libo",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToWords(tt.amount)
			if result != tt.expected {
				t.Errorf("ToWords() = %v, want %v", result, tt.expected)
			}
		})
	}
}

func TestToWords_Filipino_EdgeCases(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Large number",
			amount: 999999999,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Siyam Daan Siyamnapu Siyam Milyun Siyam Daan Siyamnapu Siyam Libo Siyam Daan Siyamnapu Siyam",
		},
		{
			name:   "Small decimal with decimal flag",
			amount: 0.01,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Decimal: true,
				},
			},
			expected: "Zero at Isa", // 0.01 = 0 and 1/100
		},
		{
			name:   "Round million",
			amount: 1000000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Milyun",
		},
		{
			name:   "Eleven hundred",
			amount: 1100,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Libo Isang daan",
		},
		{
			name:   "Twelve hundred",
			amount: 1200,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "Isang Libo Dalawa Daan",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToWords(tt.amount)
			if result != tt.expected {
				t.Errorf("ToWords() = %v, want %v", result, tt.expected)
			}
		})
	}
}

// ========== ToFormat Tests ==========

func TestToFormat_Philippines_Numbers(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Zero",
			amount: 0,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "0",
		},
		{
			name:   "Single digit",
			amount: 5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "5",
		},
		{
			name:   "Two digits",
			amount: 25,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "25",
		},
		{
			name:   "Three digits",
			amount: 100,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "100",
		},
		{
			name:   "Four digits with separator",
			amount: 1000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "1,000",
		},
		{
			name:   "Five digits with separator",
			amount: 12345,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "12,345",
		},
		{
			name:   "Six digits with separators",
			amount: 123456,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "123,456",
		},
		{
			name:   "Seven digits with separators",
			amount: 1234567,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "1,234,567",
		},
		{
			name:   "One million with separators",
			amount: 1000000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "1,000,000",
		},
		{
			name:   "Large number with separators",
			amount: 1234567890,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "1,234,567,890",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToFormat(tt.amount)
			if result != tt.expected {
				t.Errorf("ToFormat(%f) = %q, expected %q", tt.amount, result, tt.expected)
			}
		})
	}
}

func TestToFormat_Philippines_Decimals(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Decimal with one place",
			amount: 5.5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "5.5",
		},
		{
			name:   "Decimal with two places",
			amount: 5.25,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "5.25",
		},
		{
			name:   "Decimal with multiple places",
			amount: 123.456789,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "123.456789",
		},
		{
			name:   "Large number with decimals and separators",
			amount: 1234567.89,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "1,234,567.89",
		},
		{
			name:   "Small decimal",
			amount: 0.99,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "0.99",
		},
		{
			name:   "Complex decimal with separators",
			amount: 12345678.123456,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "12,345,678.123456",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToFormat(tt.amount)
			if result != tt.expected {
				t.Errorf("ToFormat(%f) = %q, expected %q", tt.amount, result, tt.expected)
			}
		})
	}
}

func TestToFormat_Philippines_Currency(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Simple peso amount",
			amount: 1234,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "₱1,234.00",
		},
		{
			name:   "Large peso amount with separators",
			amount: 1234567,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "₱1,234,567.00",
		},
		{
			name:   "Negative peso",
			amount: -1000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "-₱1,000.00",
		},
		{
			name:   "Override currency symbol",
			amount: 5000,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
					OverrideOptions: &numi18n.OverrideOptions{
						Symbol: "$",
					},
				},
			},
			expected: "$5,000.00",
		},
		{
			name:   "Zero peso",
			amount: 0,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "₱0.00",
		},
		{
			name:   "Peso with decimals",
			amount: 1234.56,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
				WordDetails: &numi18n.WordDetails{
					Currency: true,
				},
			},
			expected: "₱1,234.56",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToFormat(tt.amount)
			if result != tt.expected {
				t.Errorf("ToFormat(%f) = %q, expected %q", tt.amount, result, tt.expected)
			}
		})
	}
}

func TestToFormat_Philippines_Negative(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Negative single digit",
			amount: -5,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "-5",
		},
		{
			name:   "Negative large number with separators",
			amount: -1234567,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "-1,234,567",
		},
		{
			name:   "Negative with decimals",
			amount: -1234.56,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "-1,234.56",
		},
		{
			name:   "Negative decimal only",
			amount: -0.99,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "-0.99",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToFormat(tt.amount)
			if result != tt.expected {
				t.Errorf("ToFormat(%f) = %q, expected %q", tt.amount, result, tt.expected)
			}
		})
	}
}

func TestToFormat_Philippines_EdgeCases(t *testing.T) {
	tests := []struct {
		name     string
		amount   float64
		options  *numi18n.NumI18NOptions
		expected string
	}{
		{
			name:   "Very small positive number",
			amount: 0.001,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "0.001",
		},
		{
			name:   "Very small negative number",
			amount: -0.001,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "-0.001",
		},
		{
			name:   "Very large number with separators",
			amount: 999999999999.99,
			options: &numi18n.NumI18NOptions{
				Locale: "ph-PH",
			},
			expected: "999,999,999,999.99",
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			result := tt.options.ToFormat(tt.amount)
			if result != tt.expected {
				t.Errorf("ToFormat(%f) = %q, expected %q", tt.amount, result, tt.expected)
			}
		})
	}
}
